...
Usage:
  GoTravel route-match run [--db gotravel.sqlite] [--provider noop|ors|osrm|valhalla] [--profile value] [--ors-base-url url] [--osrm-base-url url] [--from value] [--to value] [--radius meters]
...
func addRouteMatchCommonFlags(fs *flag.FlagSet) *routeMatchCommonArgs {
	...
	fs.StringVar(&common.orsBaseURL, "ors-base-url", "", "ORS base URL")
...
func buildRouteMatchEnricher(common *routeMatchCommonArgs) (*routing.Enricher, error) {
	provider, err := providers.New(providers.Config{
		...
		ORS: providers.ORSConfig{
			BaseURL: common.orsBaseURL,
			Profile: common.profile,
		},
...
func writeRouteMatchGPX(w io.Writer, run storage.RouteMatchRun) error {
	...
	for _, coordinate := range coordinates {
		doc.Track.Segment.Points = append(doc.Track.Segment.Points, routeMatchGPXPoint{
			Lat: strconv.FormatFloat(coordinate.Lat, 'f', -1, 64),
			Lon: strconv.FormatFloat(coordinate.Lng, 'f', -1, 64),
		})
	}
